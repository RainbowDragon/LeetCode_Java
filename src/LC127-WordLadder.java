...
class Solution {
    ...
    int bfs(HashSet<String> beginSet, HashSet<String> endSet, HashSet<String> dictionary, int level) {
        ...
        if (beginSet.size() > endSet.size()) {
            return bfs(endSet, beginSet, dictionary, level);
        }
        ...
        for (String beginWord : beginSet)
            for (int i = 0; i < beginWord.length(); i++)
                ...
                {
                    char c = (char)('a' + j);
                    String tempWord = beginWord.substring(0, i) + c + beginWord.substring(i+1);

                    if (endSet.contains(tempWord)) {
                        return level+1;
                    }
                    else if (dictionary.contains(tempWord)) {
                        current.add(tempWord);
                        dictionary.remove(tempWord);
                    }
                }

        return bfs(current, endSet, dictionary, level+1);
    }
}
